...
// Function to periodically log the current transmission power and other data
void LogData(Ptr<Node> node, Ptr<WifiPhy> phy, double rssi) {
    double currentTime = Simulator::Now().GetSeconds();
    double txPowerStart = phy->GetTxPowerStart();
    csvFile << currentTime << "," << node->GetId() << "," << rssi << "," 
            << txPowerStart << "," << packetCount << "\n";
}

// Function to handle periodic logging and ADR adjustments
...
int main (int argc, char *argv[]) {
    ...
    csvFile.open("fanet_data_attack.csv");
    csvFile << "Time(s),Node,RSSI(dBm),TransmissionPower(dBm),PacketCount\n";
    ...
    //wifiPhy.EnablePcap("Fanetleader", devices.Get(0), true);
    // Enable Pcap, XML, and ASCII tracing
    wifiPhy.EnablePcap("Fanetleader", devices, true); // Enable pcap on all nodes
