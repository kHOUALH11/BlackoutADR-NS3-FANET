...
void AdjustAdr (Ptr<Node> node, Ptr<WifiNetDevice> dev)
{
    ...
    // Simulated RSSI value - this would be dynamic in a real-world scenario
    double simulatedRssi = -90 + (std::rand() % 20);  // Randomly generate RSSI between -90 and -70 dBm

    NS_LOG_UNCOND ("Node " << node->GetId() << " RSSI: " << simulatedRssi << " dBm");

    // Adjust transmission parameters based on simulated RSSI
    if (simulatedRssi < -85)
    {
...
int main (int argc, char *argv[])
{
  ...
  AnimationInterface anim ("Fanet3DLoraWan.xml");
