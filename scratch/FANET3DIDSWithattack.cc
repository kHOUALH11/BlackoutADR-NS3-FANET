...
// Function to simulate battery depletion, with more aggressive drain under attack
...
// Function to simulate RSSI changes under attack
double AdjustRssi(double originalRssi, bool underAttack) {
    return underAttack ? originalRssi - 10.0 : originalRssi;
}

// Function to periodically log transmission power, RSSI, battery level, and other data
void LogData(Ptr<Node> node, Ptr<WifiPhy> phy, double rssi, bool underAttack) {
    double currentTime = Simulator::Now().GetSeconds();
    double txPowerStart = phy->GetTxPowerStart();
    uint32_t nodeId = node->GetId();

    // Adjust RSSI if under attack and deplete battery based on current transmission power and attack status
    double adjustedRssi = AdjustRssi(rssi, underAttack);
    DepleteBattery(nodeId, txPowerStart, underAttack);

    // Log to CSV: Time, Node ID, Adjusted RSSI, Transmission Power, Packet Count, Battery Level
    csvFile << currentTime << "," << nodeId << "," << adjustedRssi << ","
            << txPowerStart << "," << packetCount << "," << batteryLevels[nodeId] << "\n";
}

// Function to handle periodic logging, ADR adjustments, and battery depletion
void PeriodicLogAndAdr(Ptr<Node> node, Ptr<WifiNetDevice> dev, bool underAttack) {
    Ptr<WifiRemoteStationManager> stationManager = dev->GetRemoteStationManager();
    Ptr<WifiPhy> phy = dev->GetPhy();

    // Simulate RSSI and adjust ADR based on it, simulating effect of attack if applicable
    double simulatedRssi = -90 + (std::rand() % 20);
    simulatedRssi = AdjustRssi(simulatedRssi, underAttack);

    if (underAttack || simulatedRssi < -85) {
        stationManager->SetAttribute("DataMode", StringValue("DsssRate1Mbps"));
        phy->SetTxPowerStart(20.0);
        phy->SetTxPowerEnd(20.0);
        NS_LOG_UNCOND("Node " << node->GetId() << " setting to low data rate with high power due to ADR attack");
    } else {
    ...
    // Log data to CSV
    LogData(node, phy, simulatedRssi, underAttack);
    
    // Schedule next logging and ADR adjustment
    Simulator::Schedule(Seconds(1.0), &PeriodicLogAndAdr, node, dev, underAttack);
}
...
int main(int argc, char* argv[]) {
    ...
    csvFile.open("fanet_data__with_attack_New.csv");
    ...
    // Schedule periodic logging and ADR adjustments with attack
    bool underAttack = true;
    for (uint32_t i = 0; i < nodes.GetN(); ++i) {
        Ptr<Node> node = nodes.Get(i);
        Ptr<WifiNetDevice> wifiDevice = DynamicCast<WifiNetDevice>(node->GetDevice(0));
        if (wifiDevice) {
            Simulator::Schedule(Seconds(1.0), &PeriodicLogAndAdr, node, wifiDevice, underAttack);
        }
    }
    ...
    wifiPhy.EnablePcap("Fanetleader2", devices, true); // Enable pcap on all nodes
